...
@TeleOp(name = "RotoTraslazionev4")
public class RotoTraslazionev4 extends LinearOpMode {
    ...
    if (opModeIsActive()) {
      ...
      while (opModeIsActive()) {
        ...
        if (verboso) {
          ...
          telemetry.addData("errore", Math.toDegrees(phi - theta));
  ...
  private void ScriviSuFile(String nomeFile, Object dati, boolean appendSeEsistente) {
    //nome del file, dato (di qualsiasi tipo), decidere se appendere o sovrascrivere in caso esista già il file
    try {
      File directory = new File("/sdcard/FIRST/data/");
      if (!directory.exists()) {directory.mkdirs();} // Crea la cartella se non esiste
      File file = new File(directory, nomeFile);
      
      PrintWriter scrittore = new PrintWriter(new FileWriter(file, appendSeEsistente));
      StringBuilder riga = new StringBuilder();
      if (dati instanceof double[]) {
        double[] arr = (double[]) dati;
        for (int i = 0; i < arr.length; i++) {
          riga.append(String.format(java.util.Locale.US, "%.2f", arr[i]));
          if (i < arr.length - 1) {riga.append("  ");}
        }
      }
      else {riga.append(dati);}
      scrittore.println(riga.toString()); //Scrive il dato (convertito automaticamente in stringa) e va a capo
      scrittore.flush();                  //Forza il salvataggio immediato sulla memoria della Hub
      scrittore.close();                  //Chiude il file per evitare di lasciarlo bloccato
    }
    catch (IOException e) {telemetry.addData("Errore File [" + nomeFile + "]", e.getMessage());} //se fallisce da errore
  }
  private void ScriviBufferSuFile(String nomeFile, ArrayList<String> buffer) {
    try {
      ...
      // false = sovrascrive il file precedente a ogni avvio del robot, così non accumula gigabyte di log vecchi
      PrintWriter scrittore = new PrintWriter(new FileWriter(file, false)); 
      for (String linea : buffer) {
          scrittore.println(linea);
      }
      scrittore.flush();
      scrittore.close();
    }
    catch (IOException e) {
      // In fase di chiusura la telemetria non è stampabile, l'eccezione evita però il crash del sistema
    }
  }
}